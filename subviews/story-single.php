...
<div class="swiper-container story-single waves-effect">
    <div class="swiper-wrapper">
        <a link="<?= home_url('/story/' . $book->ID . '/1'); ?>" href="<?= '/story/' . $book->ID . '/' . ($track ? $track['num'].'#'.($finished ? "end" : $track["para"]) : '1'); ?>" class="book swiper-slide" book_id="<?= $book->ID; ?>" chapters="<?= $chapters_count; ?>" >
            ...
            <reading-progress <?= !$track ? "" : ($finished ? "class='finished'" : "style='--perc: $track_percentage%'") ?>></reading-progress>
        </a>
