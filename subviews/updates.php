    ...
    <author-updates <?= empty($updates) ? 'empty' : '' ?>>
        <?php foreach ($updates as $update ) { ?>
            <update class="divider" <?= is_sticky( $update->ID ) ? 'pinned' : '' ?> update_id="<?= $update->ID; ?>">
                ...
                <?php } ?>
                <content><?= !in_array($user->user_login,["ffonline",'admin']) ? htmlspecialchars($update->post_content) : strip_tags($update->post_content,'<p><span><a><ul><li><ol><h1><h2><h3><h4><h5><h6>'); ?></content>
            </update>
        <?php } ?>
