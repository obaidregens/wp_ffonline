...
class book_query{
    ...
    public static function cache(){
        $books_all = (new WP_Query(array_replace(book_query::$wp_base_args,array(
            'posts_per_page'    => -1,
        ))))->posts;
        $ids = array_column($books_all,'ID');
        ...
        foreach ( $terms as $term ) {
            $term->taxonomy = $term->taxonomy === 'category' ? 'fandom' : $term->taxonomy;
            $term_key = $term->taxonomy . $midfix . $term->term_id;
        ...
        foreach ($word_limits as $from) {
            ...
            $key_value_ids['words' . $midfix . $from] = array(
                '_key'        => 'words',
                '_value'      => $from,
                'ids'        => $word_ids
            );
        ...
        foreach ($orders as $order) {
            ...
            $key_value_ids['sort' . $midfix . 'updated/' . $order] = array(
                '_key'        => 'sort',
        ...
        foreach ( $books_all as $book ) {
            $term_key = 'author' . $midfix . $book->post_author;
            if (! isset($key_value_ids[$term_key])){
                $key_value_ids[$term_key] = array(
                    '_key'       => 'author',
