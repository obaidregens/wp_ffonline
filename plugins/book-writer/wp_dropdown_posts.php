...
if( ! function_exists( 'wp_dropdown_posts' ) ) {
	 ...
	 *     @type bool         $direct                  Set true to direct to post in dropdown menu, default false. Parameter added by www.fanfiction.online
	 *     @type bool         $number                  Set true to number posts in the dropdown menu, default false. Parameter added by www.fanfiction.online
	 * }
	...
	function wp_dropdown_posts( $args = '' ) {
		...
		if( ! empty($posts) ) {
			...
			$output = "<select name='{$name}'{$id} class='" . esc_attr( $r['class'] ) . "'>n";
			
            if ($r['direct']) $output = "<select onchange='window.location=this.options[this.selectedIndex].value' name='{$name}'{$id} class='" . esc_attr( $r['class'] ) . "'>n";

			if( $r['show_option_all'] ) {
				$output .= "t<option value='0'>{$r['show_option_all']}</option>n";
			}

			if( $r['show_option_none'] ) {-
				$_selected = selected( $r['show_option_none'], $r['selected'], FALSE );
				$output .= "t<option value='" . esc_attr( $r['option_none_value'] ) . "'$_selected>{$r['show_option_none']}</option>n";
			}
			...
			foreach( (array) $posts as $post ) {
                ...
                if ($r['direct']) $value   = get_permalink(! isset($r['value_field']) || ! isset($post->{$r['value_field']}) ? $post->ID : $post->{$r['value_field']});
				...
				if ( $r['number'] ) {$output .= "t<option value='{$value}'{$_selected}>" . $number_list . ". " . esc_html( $display ) . "</option>n";}
				else {$output .= "t<option value='{$value}'{$_selected}>" . esc_html( $display ) . "</option>n";}
			}
