...
class collection_follow extends collection {
    public static function is($collection_id,$user_id = null){
        if ($user_id === null){
            $user_id = get_current_user_id();
        }
        $collection_ids = array_column(
            collection::follower_query(array(
                'user_ids'      => array($user_id)
            )),
            'collection_id'
        );
        if (in_array(strval($collection_id),$collection_ids)){
            return true;
        }
        return false;
    }
    public static function follow($collection_id, $user_id = null){
        if ($user_id === null){
            $user_id = get_current_user_id();
        }
        collection::add_follow($collection_id, $user_id, 'no-email');
    }
    public static function unfollow ($collection_id, $user_id = null){
        if ($user_id === null){
            $user_id = get_current_user_id();
        }
        collection::add_follow($collection_id, $user_id, false);
    }
}
$all_collection_query_fields = 
[
    'text_search_protocol'  =>      'lenient'/* strict */,
    'included'/* excluded */=>      [
        'types' => ['Public','Unlisted','Private','Favorites','Trash'],
        'ids'   => [/** Any */]
    ]
];
// Some Ground Rules
// Slugs are
    // Public, Private, Favorites
        // (_, ,.) replaced by hyphens(-) in lowercase titles
    // Unlisted
        // Automatically random generated
// Slugs have to be 
    // unique for each user
    // Public, Unlisted
        // unique for all
// Modified is used for when any book in collection is updated
// Delete Collections should be assigned type: Trash