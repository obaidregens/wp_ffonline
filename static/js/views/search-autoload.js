        ...
        if (page_ids[toRemove]) {
            ...
            DOM.qa('a.book').forEach(book => {
                ...
                if (response.ids.includes(b_id)) {
                    console.log(book.parentElement.parentElement.clientHeight,book.parentElement.parentElement.scrollHeight);
...
window.addEventListener("scroll",evt => {
    ...
    if (Object.keys(page_ids).filter(pg => int(pg) > 0 && pg <= max_pages).length < 1) {
        return;
    }

    loading_page = true;
    const books_in_view = im('books').map(parseInt);
    let pages_in_view = [];
    books_in_view.forEach(book_id_in_view => {
        Object.entries(page_ids).forEach( ([page_in,book_ids_of_page]) => {
            book_ids_of_page = book_ids_of_page.map(int);
            if ( book_ids_of_page.includes(book_id_in_view) ) {
                pages_in_view.push(int(page_in));
            }
        });
    });
